...
export function Switch({ checked, defaultChecked, disabled = false, onChange, label = '', style = {} }) {
  ...
  return (
    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-3)', cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.55 : 1, ...style }}>
      <span
        onClick={() => { if (disabled) return; if (!isControlled) setInternal(!on); onChange && onChange(!on); }}
        style={{
          width: 46, height: 26, flex: '0 0 46px', borderRadius: 'var(--radius-pill)', position: 'relative',
      ...
      >
        <span style={{
          position: 'absolute', top: 2, left: on ? 22 : 2, width: 20, height: 20, borderRadius: 'var(--radius-pill)',
          background: on ? 'var(--emerald-950)' : 'var(--gold-100)', boxShadow: 'var(--shadow-xs)',
