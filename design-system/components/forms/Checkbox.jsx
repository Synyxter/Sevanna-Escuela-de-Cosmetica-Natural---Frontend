  ...
  return (
    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-3)', cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.55 : 1, ...style }}>
      <span
        ...
        style={{
          width: 22, height: 22, flex: '0 0 22px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
          borderRadius: 'var(--radius-xs)',
          border: `1px solid ${on ? 'transparent' : 'var(--border-strong)'}`,
          background: on ? 'var(--gradient-gold)' : 'var(--surface-inset)',
          color: 'var(--text-on-gold)', transition: 'background var(--dur-base) var(--ease-standard)',
        }}
      >
